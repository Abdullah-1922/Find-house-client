      ...
      <div className="relative">
        ...
        <div className="flex justify-start mb-5 rounded-lg p-1  backdrop-blur-sm shadow-sm gap-5">
          <button
            ...
            className={cn(
              "px-6 py-3 font-bold rounded-lg text-sm font-medium transition-colors",
              searchType === 'sale' 
      ...
      {isOpenProperty && (
        <div className="absolute w-full mt-2 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
          <ul className="py-2 text-left">
            <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-gray-700">Apartment</li>
            <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-gray-700">Family House</li>
            <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer text-gray-700">Condo</li>
          </ul>
        </div>
      )}
    </div>
      ...
      {isOpen && (
        <div className="absolute w-full mt-2 bg-white rounded-lg shadow-lg border border-gray-200 z-10">
          <ul className="py-2  text-gray-500 text-left" >
            <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer">Rajshahi</li>
            <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer">Rongpur</li>
            <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer">Dhaka</li>
            <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer">Pabna</li>
            <li className="px-4 py-2 hover:bg-gray-100 cursor-pointer">Sylet</li>
          </ul>
        </div>
      )}
    </div>
          ...
          <div className="relative">
            ...
            >
                Advanced Search
                <Image className='w-5' src={menuicon} alt='world svg for language' width={1000} height={1000}/>
            </button>
    ...
    {isOpenAdvance && (
     ...
     <div className="space-y-6 w-full" >
        <div className="space-y-4">
          <label className="block text-sm font-medium text-gray-700 text-left">Area Size</label>
        ...
        <div className="space-y-4">
          <label className="block text-sm font-medium text-gray-700 text-left">Price Range</label>
          <div className="space-y-2">
            <div className="relative">
              <input
                type="range"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-4 w-full">
        <div className="space-y-4">
          {['Air Conditioning', 'Swimming Pool', 'Central Heating', 'Laundry Room', 'Gym', 'Alarm', 'Window Covering'].map((amenity, index) => (
            <div key={index} className="flex items-center space-x-2">
              <input type="checkbox" id={amenity.toLowerCase().replace(' ', '-')} className="rounded border-gray-300 text-gray-600 focus:ring-gray-600" defaultChecked={index < 4} />
              <label htmlFor={amenity.toLowerCase().replace(' ', '-')} className="text-sm text-gray-700">{amenity}</label>
            </div>
          ))}
        </div>
        <div className="space-y-4">
          {['WiFi', 'TV Cable', 'Dryer', 'Microwave', 'Washer', 'Refrigerator', 'Outdoor Shower'].map((amenity, index) => (
            <div key={index} className="flex items-center space-x-2">
              <input type="checkbox" id={amenity.toLowerCase().replace(' ', '-')} className="rounded border-gray-300 text-gray-600 focus:ring-gray-600" defaultChecked={index === 0} />
              <label htmlFor={amenity.toLowerCase().replace(' ', '-')} className="text-sm text-gray-700">{amenity}</label>
            </div>
          ))}
        </div>
